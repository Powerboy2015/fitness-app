    ...
    return (
        <footer className="relative bottom-0 z-48 py-2 w-full bg-navbar border-t-2 border-bordercolor">
            <nav className="h-full max-w-sm mx-auto justify-evenly flex">
                <NavLink to="/" className={({ isActive }) => `text-center items-center ${isActive ? 'text-orange-accent' : 'text-icons'}`}>
                    <HomeSharpIcon sx={{ fontSize: 40 }}/>
                    <p className="text-xs">Home</p>
                </NavLink>
                <NavLink to="/workouts" className={({ isActive }) => `text-center items-center ${isActive ? 'text-orange-accent' : 'text-icons'}`}>
                    <FitnessCenterIcon sx={{ fontSize: 40 }}/>
                    <p className="text-xs">Workout</p>
                </NavLink>
                <NavLink to="/kcal-tracker" className={({ isActive }) => `text-center items-center ${isActive ? 'text-orange-accent' : 'text-icons'}`}>
                    <RestaurantSharpIcon sx={{ fontSize: 40 }}/>
                    <p className="text-xs">Kcal</p>
                </NavLink>
                <NavLink to="/profile" className={({ isActive }) => `text-center items-center ${isActive ? 'text-orange-accent' : 'text-icons'}`}>
